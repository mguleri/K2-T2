...
We are using the ggplot2 package (part of the tidyverse) for visualizing the data.
Learn more about the package here: https://ggplot2.tidyverse.org/

In most cases, you start with `ggplot()`, supply a dataset and aesthetic mapping (with `aes()`), and then add on other layers (`geom_point()` or `geom_histogram()`) and specifications.

```{r ggplot examples, echo=FALSE}
